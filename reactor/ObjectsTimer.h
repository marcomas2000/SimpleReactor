...
#ifndef SYSTEM_REACTOR_REACTOR_OBJECTSTIMER_H_
#define SYSTEM_REACTOR_REACTOR_OBJECTSTIMER_H_
...
namespace reactor
{
  ...
  ObjectsTimer<T>::get_min_timeout_milliseconds_no_mutex()
  {
    ...
    int32_t min = std::chrono::duration_cast<std::chrono::milliseconds>(
                      time_object_.begin()->first - std::chrono::steady_clock::now())
                      .count();
...
#endif // SYSTEM_REACTOR_REACTOR_OBJECTSTIMER_H_
